...
class AStarPathfinder:
    ...
    def move_robot_along_path(self, motor_controller, path, gyro, segment_length=10):
        """
        Move the robot along the path generated by the A* algorithm.
        :param motor_controller: MotorController object with control methods.
        ...
        :param segment_length: Length of each segment to divide the path into.
        """
        ...
        for target_position in path[1:]:
            ...
            target_angle, total_distance = self.calculate_angle_and_distance(current_position, target_position)

            # Determine the number of segments
            num_segments = int(math.ceil(total_distance / segment_length))

            for i in range(num_segments):
                # Calculate intermediate point
                ratio = (i + 1) / num_segments
                intermediate_position = (
                    current_position[0] + ratio * (target_position[0] - current_position[0]),
                    current_position[1] + ratio * (target_position[1] - current_position[1])
                )

                # Calculate angle and distance to the intermediate point
                segment_angle, segment_distance = self.calculate_angle_and_distance(current_position, intermediate_position)

                # Get current angle from gyroscope
                current_angle = gyro.get_angle_z()

                # Calculate the rotation required
                angle_to_rotate = segment_angle - current_angle

                # Choose the direction of rotation
                direction = 'left' if angle_to_rotate < 0 else 'right'

                # Rotate the robot to face the segment angle
                motor_controller.rotate_to_angle(gyro, target_angle=segment_angle, direction=direction)
